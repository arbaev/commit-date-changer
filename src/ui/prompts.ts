...
`;
    ...
    type ChoiceItem =
      | { name: string; value: Commit; short: string }
      | inquirer.Separator;
    ...
    if (allowPushed && pushed.length > 0 && unpushed.length > 0) {
      ...
      choices.splice(
        unpushedCount + 1,
        0,
        new inquirer.Separator(chalk.red('═══ ЗАПУШЕННЫЕ') + chalk.yellow(' (⚠️  опасно изменять)') + chalk.red(' ═══'))
      );
    }
    ...
    const answer = await inquirer.prompt([
      {
        ...
        validate: (input: string) => {
          // Валидация формата
          const formatValidation = this.validator.validateISOFormat(input);
          if (!formatValidation.isValid) {
            return formatValidation.error || 'Невалидная дата';
          }

          // Парсинг и валидация диапазона
          const parsedDate = this.validator.parseDate(input);
          if (!parsedDate) {
